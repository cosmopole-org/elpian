...
class HtmlH1 {
  static Widget build(StacNode node, List<Widget> children) {
    ...
    // Merge defaults with custom style (custom overrides defaults)
    final mergedStyle = CSSStyle(
      fontSize: node.style?.fontSize ?? 32.0,
      fontWeight: node.style?.fontWeight ?? FontWeight.bold,
      margin: node.style?.margin ?? const EdgeInsets.symmetric(vertical: 16.0),
