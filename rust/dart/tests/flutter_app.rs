...
fn demo_app_renders_and_is_interactive() {
    ...
    assert!(
        t0.contains(&"Elpian Dashboard".to_string()),
        "app bar title: {t0:?}"
    );
    assert!(
        t0.contains(&"COUNTER".to_string()),
        "counter card label: {t0:?}"
    );
    ...
    assert_eq!(
        t0.iter().filter(|s| *s == "0").count(),
        4,
        "counter + 3 chips = 0: {t0:?}"
    );
    ...
    let t1 = texts(&f1);
    assert!(
        t1.contains(&"3".to_string()),
        "counter -> 3 after 3 taps: {t1:?}"
    );
    ...
    assert!(
        texts(&f2).contains(&"2".to_string()),
        "counter -> 2 after a decrement"
    );
}
...
fn green_button_center(frame: &Value, plus: bool) -> (f64, f64) {
    ...
    panic!(
        "no {} button rect found",
        if plus { "green" } else { "red" }
    );
}
